...
- [x] 10.1 Validação via curl contra os servidores rodando: onboarding de condomínio ✓ → login admin retorna `role: "admin"` ✓ → cria/edita/desativa um `CommonArea` via API (mesmos endpoints que a UI admin usa) ✓ → convite de prestador já sai com `role: "service_provider"` (confirma o rename ponta a ponta) ✓ → login morador retorna `role: "resident"` ✓ → login prestador retorna `role: "service_provider"` ✓ → ambos listam `CommonArea` normalmente (inclui inativo, com status visível) ✓ → prestador tentando criar `CommonArea` é rejeitado (422 `unauthorized`, admin-only mantido) ✓. Renderização em navegador (sidebar por persona, tabela vs cards, modal) coberta pelos 55 testes de componente do Grupo 8 — curl não executa JS, essa camada foi validada por lá, não aqui.
