...
Capability introduzida por `add-common-area-context`. Catálogo de espaços
comuns do condomínio (`CommonArea`): `nome`, `descricao`, `capacidade`,
`horario_funcionamento`, `regras_uso`, `ativo`. Recurso do condomínio
inteiro, sem vínculo com nenhuma `Registry::Building`. Cadastro e edição
restritos a admin; edição é livre (sem imutabilidade, diferente de
`Billing::Taxa`/`Notice::Aviso`). Listagem aberta a qualquer `Membership`
ativo do condomínio, incluindo espaços com `ativo: false` (indisponibilidade
temporária, não um registro incorreto — por isso não é ocultado). Sem
sistema de reservas nesta v1; quando existir, deve ser um bounded context
próprio (`Reservation`, candidato), lendo `CommonArea` como Open Host
Service.
