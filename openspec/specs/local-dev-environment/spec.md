...
TBD — capability introduced by `migrate-skeleton-to-monolith`. Define a topologia de
ambiente de desenvolvimento local do Neighbry: um monolito Rails único (`neighbry-api`)
com Postgres único, Sidekiq para jobs, mocks de integração internos ao processo, sem
gateway HTTP, sem mensageria assíncrona entre serviços, sem infraestrutura de e-mail e
sem suíte E2E Playwright.
