...
Capability introduzida por `add-reservation-context`. Reserva de espaços
comuns (`Reservation::Booking`) por dono ou responsável ativo de uma
`Registry::Unit`, num turno fixo (`manha`/`tarde`/`noite`), dentro de uma
janela de 30 dias a partir de hoje. Aprovação automática — sem etapa de
aprovação por admin nesta v1. Cancelamento self-service, restrito a quem
reservou; cancelamento por admin fica fora de escopo (backlog v2). Sem
cobrança nesta v1 — reservar é gratuito. Duas invariantes de concorrência
garantidas por índice único parcial no banco: no máximo uma `Booking`
ativa por `CommonArea` + `data` + `turno`, e no máximo uma `Booking` ativa
por `Unit` + `CommonArea` + mês (limite de justiça de uso, independente de
turno). Listagem aberta a qualquer `Tenancy::Membership` ativo do
condomínio. Bloqueio pontual de turno por manutenção fica fora de escopo
(backlog v2 — hoje só existe `CommonArea#ativo`, que desliga o espaço
inteiro).
