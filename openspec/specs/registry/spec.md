...
Capability introduzida por `add-registry-context`. Estrutura física e social
de um condomínio: `Building`, `Unit`, `Person` (reconciliada por CPF dentro
do condomínio) e `Occupancy` (vínculo entre `Person` e `Unit`, com papéis
`owner`/`responsible` mutuamente exclusivos). Define a hierarquia de
autorização por unidade (admin > owner > responsible > morador comum) e
como a concessão de acesso ao sistema reaproveita o fluxo de convite já
existente em `Tenancy`, sem duplicar lógica de segurança.
